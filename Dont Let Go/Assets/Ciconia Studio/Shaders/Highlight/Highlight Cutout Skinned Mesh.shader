...
Shader "Ciconia Studio/Effects/Highlight/Skinned Mesh (No Script)/Cutout" {
    Properties {
        ...
        [Space(45)][Header(Highlight Properties)]
        [Space(15)][MaterialToggle] _NormalDirection ("Normal Direction", Float ) = 0
        [Space(15)]_HighlightColor ("Color", Color) = (1,1,1,1)
        _FresnelSpread ("Fresnel Spread", Range(0, 5)) = 3
        _FresnelStrength ("Fresnel Intensity", Range(0, 8)) = 5
        [Space(25)][MaterialToggle] _OutlineOnOff ("Outline On/Off", Float ) = 0
        [Space(15)]_OutlineColor ("Outline Color", Color) = (0,0,0,1)
    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_5425 = 0.0;
                float OutlineWidth = lerp( node_5425, lerp(node_5425,0.05,_OutlineWidth), _OutlineOnOff );
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float AlphaClip = lerp( _MainTex_var.a, (1.0 - _MainTex_var.a), _InvertCutout );
                clip(AlphaClip - 0.5);
        ...
        Pass {
            ...
            uniform fixed _NormalDirection;
            uniform fixed _InvertCutout;
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float3 Normal = lerp(float3(0,0,1),_BumpMap_var.rgb,_NormalIntensity);
...
////// Specular:
                ...
                float3 node_8824 = (_Color.rgb*lerp(_MainTex_var.rgb,dot(_MainTex_var.rgb,float3(0.3,0.59,0.11)),_Desaturate));
                float3 Albedo = node_8824;
...
/////// Diffuse:
                ...
                float node_2862 = Ambient;
                indirectDiffuse += float3(node_2862,node_2862,node_2862); // Diffuse Ambient Light
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : SV_Target {
                ...
                float4 _EmissionMap_var = tex2D(_EmissionMap,TRANSFORM_TEX(i.uv0, _EmissionMap));
                float3 Emissive = (_EmissionColor.rgb*_EmissionMap_var.rgb*_EmissionIntensity);
                o.Emission = (FresnelEmissive+Emissive);
